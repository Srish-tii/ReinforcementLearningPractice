...
class A2C(AbstractSolver):
    ...
    def train(self, states, actions, rewards, next_state, done):
        ...
        # and actions
        future_return = 0 if done else self.actor_critic.value(
            torch.tensor(next_state, dtype=torch.float32)).item()

        for i in reversed(range(len(rewards))):
            future_return = rewards[i] + self.options.gamma * future_return
